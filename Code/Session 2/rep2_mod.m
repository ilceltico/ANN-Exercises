...
%Diagonal points (positive slope)
n=20;
for i=1:n
    temp=2/n * i -1;
    a={[temp;temp]};                     % generate an initial point 
    [y,Pf,Af] = sim(net,{1 50},{},a);   % simulation of the network for 50 timesteps              
    record=[cell2mat(a) cell2mat(y)];   % formatting results  
    start=cell2mat(a);                  % formatting results 
    plot(start(1,1),start(2,1),'bx',record(1,:),record(2,:),'r'); % plot evolution
...
%Diagonal points (negative slope)
n=20;
for i=1:n
    temp=2/n * i -1;
    a={[temp;-temp]};                     % generate an initial point 
...
%So no, the attractors are not 4 + unstable point, there are also other 4!